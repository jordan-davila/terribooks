        ...
        >
            <div class="popper rounded-md shadow-lg w-60 bg-white overflow-hidden z-50">
                <div class="rounded-md ring-1 ring-black ring-opacity-5">
                    ...
                    <inertia-link
                        preserve-scroll
                        preserve-state
                        :href="route('territories.editor.field.show', [$page.props.territory.data.id, $page.props.street.id])"
                    ...
                    <inertia-link
                        preserve-scroll
                        preserve-state
                        :href="route('territories.editor.phone.show', [$page.props.territory.data.id, $page.props.street.id])"
                        class="block px-4 py-2 text-xs leading-5 text-gray-300 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
                    >
                        Phone
                    </inertia-link>
                    ...
                    <inertia-link
                        preserve-scroll
                        preserve-state
                        :href="route('territories.editor.business.show', [$page.props.territory.data.id, $page.props.street.id])"
                        class="block px-4 py-2 text-xs leading-5 text-gray-300 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
                    >
                        Business
                    </inertia-link>
            ...
            >
                <div class="type">
                    ...
                    <span class="text-indigo-600">Field</span>
                </div>
                <div class="w-6 h-6 text-gray-300 text-sm">
            ...
            <div class="street-list w-full px-14">
                <div class="wrapper pb-8">
                    <template v-for="street in $page.props.territory.data.streets">
                        <div class="text-xxs text-gray-700 w-full py-3 font-bold flex justify-between items-center" :key="street.id">
                            <div class="flex flex-start items-center">
                                <button class="w-5 h-5 text-gray-300 text-sm mr-4">
                                    <i class="fas fa-bars handle"></i>
                                </button>
                                <!-- *TODO* -->
                                <!-- Make a territories.editor.field.street route -->
                                <!-- Pass in street id to this route -->
                                <inertia-link
                                    preserve-scroll
                                    preserve-state
                                    :id="'street-link-' + street.id"
                                    :class="$page.props.street.id == street.id ? 'text-indigo-600 font-black' : 'text-gray-700 font-bold'"
...
export default {
    ...
    methods: {
        scrollToStreet() {
            let child = document.querySelector("#street-link-" + this.$page.props.street.id);
